...
public class CartController {
    ...
    public ResponseEntity<CartResponseDTO> updateQuantity(@PathVariable int foodId, @RequestParam int quantity, Authentication authentication) {
        String email = authentication.getName();
        User user = userService.getUserByEmail(email);
        if (user == null) {
            throw new UserNotFoundException("User not found with email: " + email);
    ...
    public ResponseEntity<String> removeFromCart(@PathVariable int foodId, Authentication authentication) {
        String email = authentication.getName();
        User user = userService.getUserByEmail(email);
        if (user == null) {
            throw new UserNotFoundException("User not found with email: " + email);
